...
class AllowedValues(Observer):
    ...
    def _addValue(self, value):
        if not SCS.IsClear(value):
            self._refCount[value] -= 1
            if self._refCount[value] < 0:
                raise ValueError(SCS.INVALIDREFCOUNTEXCEPTION + ' {} (value {})'.format(self._refCount[value], value))
    ...
    def IsAllowedValue(self, value):
        return value == SCS.INITIAL or value in self._allowedValues
